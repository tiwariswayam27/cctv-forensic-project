  ...
  evidence: [
    {
      ...
      metadata: {
        duration: "00:30:00",
        duration_sec: 1800,
        resolution: "1920 \xD7 1080 (Full HD)",
        fps: 25,
...
var FASTAPI_URL = process.env.FASTAPI_URL || "http://127.0.0.1:8000";
...
app.post("/api/cases", (req, res) => {
  ...
  const newCase = {
    ...
    case_number: case_number || `CR-${Math.floor(1e4 + Math.random() * 9e4)}`,
    case_name,
...
var handleEvidenceUpload = (req, res) => {
  const {
    case_id,
    cctv_id,
    file_name,
  ...
  if (!calculatedHash) {
    const randomBuffer = import_crypto.default.randomBytes(64);
    calculatedHash = import_crypto.default.createHash("sha256").update(file_name + Date.now() + randomBuffer.toString("hex")).digest("hex");
  }
  const evId = `EVD-00${db.evidence.length + 1}`;
  const now = acquisition_date || (/* @__PURE__ */ new Date()).toISOString().replace("T", " ").substring(0, 19);
  const newEvidence = {
    evidence_id: evId,
    case_id,
...
app.post("/api/evidence/:id/verify", (req, res) => {
  ...
  const auditEntry = {
    ...
    details: isMatch ? "Cryptographic SHA-256 hash matches the baseline acquisition digest exactly. 0 bits altered." : "CRITICAL: Current binary digest does NOT match baseline hash! Evidence has been altered or corrupted.",
...
app.get(["/api/events", "/api/analysis-events"], (req, res) => {
  ...
  const list = evidence_id ? db.analysis_events.filter((a) => a.evidence_id === evidence_id) : db.analysis_events;
  res.json({ success: true, data: list });
});
...
app.post("/api/reports/generate", (req, res) => {
  ...
  const targetEvents = db.analysis_events.filter(
    (a) => targetEvidence.some((e) => e.evidence_id === a.evidence_id)
  );
  const report = {
    ...
    generated_at: (/* @__PURE__ */ new Date()).toISOString().replace("T", " ").substring(0, 19),
    ...
    certification_statement: "I hereby certify under penalty of perjury that the digital video recordings and forensic extractions described herein have been handled, hashed, analyzed, and preserved in strict adherence to forensic video analysis standards and ISO/IEC 27037:2012 digital evidence guidelines."
  };
  res.json(report);
});
    ...
    reconstructed_files: [
      {
        ...
        size: "34.2 MB",
        duration: "00:08:45",
        sha256: import_crypto.default.createHash("sha256").update(`carved-primary-${Date.now()}`).digest("hex"),
      ...
      },
      {
        ...
        size: "22.8 MB",
        duration: "00:05:12",
        sha256: import_crypto.default.createHash("sha256").update(`carved-sec-${Date.now()}`).digest("hex"),
        integrity: "Reconstructed & Validated"
      }
    ]
  };
